...
float CSettings::ConvertFloatToUVSpace(const AXIS sAxis, const int iIndex, const bool bInvert)
{
	float fResult = 0.0f;
	if (sAxis == x)
	{
	...
	{
		if (bInvert)
			fResult = 1.0f - (float)(iIndex + 1) * TILE_HEIGHT + 0.05;
		else
			fResult = -1.0f + (float)iIndex * TILE_HEIGHT + 0.05;
	}
	else if (sAxis == z)
	{
		// Not used in here
